...
def build_summary_catalog() -> str:
    ...
    lines: list[str] = [
        ...
        "",
        f"Total registered summaries: **{len(SUMMARY_SPECS)}**",
        "",
        "| Summary ID | Filename | Builder | Output schema | Required inputs |",
        "|---|---|---|---|---|",
    ]

    for spec in sorted(SUMMARY_SPECS, key=lambda item: item.summary_id):
        contract = get_summary_contract(spec.builder)
        builder_name = (
            f"{spec.builder.__module__}.{getattr(spec.builder, '__name__', '<callable>')}"
        )
        if contract is None:
            schema = "_No contract declared_"
            required = "-"
        else:
            schema = "<br>".join(
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    f"`{_escape_cell(spec.summary_id)}`",
                    f"`{_escape_cell(spec.filename)}.csv`",
