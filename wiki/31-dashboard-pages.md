...
Dashboard pages are registered from modules under
[`dashboard/pages`](../dashboard/pages). Each page module exports a
`DashboardPageDefinition`, and page groups export a `DashboardGroupDefinition`.
...
| `title` | Display title. |
| `page_cls` | `DashboardPage` subclass. |
...
You can select a whole group or individual leaf pages, depending on the
...
Total registered pages: **26**

| Page ID | Title | Group | Default | Prepared data | Required summaries | Required prepared tables |
|---|---|---|---|---|---|---|
...
| `traffic` | Traffic Validation | Validation Summaries | yes | `none` | `traffic_count_comparisons`, `screenline_flow_comparisons` | - |
...
| `vmt` | VMT Validation | Validation Summaries | yes | `none` | `commercial_vmt_totals`, `bicycle_vmt_by_facility_type` | - |
