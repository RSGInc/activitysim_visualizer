...
3. Apply CLI flags such as `--prepare-only`, `--summarize`, `--dashboard`, and
   `--from-csvs`.
...
| `--write-csvs` | Force summary cache/CSV writes during summarize. |
...
```text
<summary_root_parent>/prepared_cache/
  <run_key>/
    manifest.json
...
`--from-csvs` reads visualizer summary-cache directories and their manifests;
it is not a loose-CSV loader. Use `runs[*].summary_table_map` for loose
