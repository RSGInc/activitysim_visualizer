...
This page documents the current canonical config layout. Older configs may
still load with migration warnings, but deprecated keys are not listed here.
...
| `summary_categories` | mapping | `{}` plus escort defaults | Summary | Advanced category normalization used while building summaries. |
| `student_types` | list of mappings | `[]` | Prepare, Summary | Advanced student-type definitions for school-related prepared fields and summaries. |
...
| Field | Type | Default | Impact | Notes |
|---|---|---|---|---|
| `enabled` | boolean | `true` when `defaults` is present, otherwise `false` | Runtime, Prepare | Explicit on/off switch. The `pipeline.steps` list still controls whether the step is run. |
| `config_path` | path string | none | Prepare, Summary | Direct skimjoin config path. Prefer `defaults.config_path` when also setting shared skim files. |
...
Run-level `runs[*].skimjoin` supports the same override fields except
`enabled`: `config_path`, `skim_files`, and `network_los_file`.
...
`summary_categories` uses the same category shape as `display.labels`, but it
changes the normalized values written into summary outputs rather than only the
dashboard label shown to users. Use it sparingly because it changes summary
cache identity.
...
`student_types` customizes school-related prepared fields and summaries.
