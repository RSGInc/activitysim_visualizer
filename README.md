...
| `dashboard.include_notes` | Show per-plot/table notes in the live dashboard and HTML export (default: `true`) |
...
```yaml
dashboard:
  # Set to false to omit all per-plot and per-table Notes controls.
  include_notes: true
  live:
    pages:
      - overview
