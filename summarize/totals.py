...
def system_totals(rd: RunData, config: Config | None = None) -> pl.DataFrame:
    ...
    emp_col = next((c for c in ["EMPLOY_TOT", "TOTEMP", "total_employment", "employment"] if c in rd.land_use.columns), None)
    ...
    pmt = (rd.trips
           .with_columns((pl.col("od_dist") * pl.col("finalweight")).alias("pmt_w"))
           ["pmt_w"].sum())
    ...
    if "trip_mode" in trips_df.columns:
        ...
        if auto_modes is not None:
            ...
            auto_filter = (
                pl.col("trip_mode").cast(pl.Utf8).str.to_uppercase()
    ...
    vmt = (auto_trips
           .with_columns((pl.col("od_dist") * pl.col("finalweight") / pl.col("num_participants").fill_null(1)).alias("vmt_w"))
           ["vmt_w"].sum())
