...
3. `dashboard.export.serializer.serialize_viewable()` converts supported Panel objects into a JSON-safe node tree.
...
### 4. Verify missing-data behavior
...
- the page falls back to static export behavior for that selector
...
- `tests/test_export_payload.py` for payload shape, page descriptors, selector metadata, and page variant structure
