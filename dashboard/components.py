...
def _layout(fig: go.Figure, title: str, xaxis_title: str, yaxis_title: str, height: int, barmode: str | None = None) -> None:
    layout_kwargs = dict(
...
def bar_chart(
    ...
    for i, (label, df) in enumerate(data_list):
        ...
        fig.add_trace(go.Bar(
            name=label, x=x, y=y_list, marker_color=run_color(i),
...
def kpi_box(label: str, values: list[tuple[str, float]], format_fn=None, icon: str = "") -> pn.viewable.Viewable:
    """
...
def get_standard_df(
    ...
    for label, df in data_list:
        if df is None:
            ...
            df = base.join(df.select([key_col, val_col]), on=key_col, how="left").fill_null(0)
        result.append((label, df.sort(key_col)))
    return result


def purpose_selector(purposes: list[str], name: str = "purpose_select") -> pn.widgets.Select:
    return pn.widgets.Select(name=name, options=purposes, value=purposes[0])


def district_selector(districts: list[str], name: str = "district_select") -> pn.widgets.Select:
