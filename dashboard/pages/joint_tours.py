...
def build(runs: list[tuple[str, RunData]], config: Config) -> pn.viewable.Viewable:
    ...
    def _ordered_comp(df: pl.DataFrame) -> pl.DataFrame:
        if len(df) == 0:
            return df
        d = df.with_columns(pl.col("tour_composition").cast(pl.Utf8).str.to_lowercase().alias("tour_composition"))
        d = d.with_columns(
            ...
            .otherwise(99).alias("_ord")
        ).sort("_ord").drop("_ord")
        return d
    comp_list = [(l, _ordered_comp(tour_sums.joint_composition(rd))) for l, rd in runs]
    party_list = [(l, tour_sums.joint_party_size(rd).with_columns(pl.col("NUMBER_HH").cast(pl.Utf8))) for l, rd in runs]
    hhsize_list = [(l, tour_sums.joint_tours_hhsize(rd)) for l, rd in runs]
    
    jtf_chart = bar_chart(
        jtf_list, x_col="alt_name", y_col="freq",
        title="Joint Tour Frequency (21 alternatives)", xaxis_title="Alternative", yaxis_title="Households",
        height=450,
    )
    
    comp_chart = bar_chart(
        comp_list, x_col="tour_composition", y_col="freq",
        title="Joint Tour Composition", xaxis_title="Composition", yaxis_title="Tours",
    )
    
    party_chart = bar_chart(
        party_list, x_col="NUMBER_HH", y_col="freq",
        title="Joint Tour Party Size", xaxis_title="Party Size", yaxis_title="Tours",
    )
    ...
    def _ordered_presence(df: pl.DataFrame) -> pl.DataFrame:
        ...
        d = (df.with_columns(pl.col("jointTours").cast(pl.Utf8).alias("jointTours"))
             .group_by("jointTours")
             .agg(pl.col("freq").sum().alias("freq")))
        return (base.join(d, on="jointTours", how="left")
    ...
    def hhsize_chart(hhsize):
        if hhsize == "Total":
            ...
            data = [(l, _ordered_presence(df.filter(pl.col("hhsize").cast(pl.Utf8) == hhsize)))
                    for l, df in hhsize_list]
        return bar_chart(data, "jointTours", "freq",
                         f"Joint Tour Presence by HH Size ({hhsize})", "Joint Tours", "% HH")
