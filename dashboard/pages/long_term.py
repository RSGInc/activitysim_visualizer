...
def build(runs: list[tuple[str, RunData]], config: Config) -> pn.viewable.Viewable:
    ...
    auto_own_list = [
        (l, demographics.auto_ownership(rd).with_columns(pl.col("HHVEH").cast(pl.Utf8)))
        for l, rd in runs
    ]
    ...
    if config.geography_enabled:
        ...
        geo_groups = (
            ["Total"] + [c for c in first_tlfd.columns if c not in ("distbin", "Total")]
            if first_tlfd is not None
            else ["Total"]
        )
        ...
        def tlfd_charts(geo):
            ...
            work_data = [
                (l, tlfs["work"].select(["distbin", col]).rename({col: "freq"}))
                for l, tlfs in tlfd_list
                if col in tlfs["work"].columns
            ]
            univ_data = [
                (l, tlfs["univ"].select(["distbin", col]).rename({col: "freq"}))
                for l, tlfs in tlfd_list
                if col in tlfs["univ"].columns
            ]
            schl_data = [
                (l, tlfs["schl"].select(["distbin", col]).rename({col: "freq"}))
                for l, tlfs in tlfd_list
        ...
        def tlfd_charts_static():
            work_data = [
                (l, tlfs["work"].select(["distbin", "Total"]).rename({"Total": "freq"}))
                for l, tlfs in tlfd_list
            ]
            univ_data = [
                (l, tlfs["univ"].select(["distbin", "Total"]).rename({"Total": "freq"}))
                for l, tlfs in tlfd_list
            ]
            schl_data = [
                (l, tlfs["schl"].select(["distbin", "Total"]).rename({"Total": "freq"}))
                for l, tlfs in tlfd_list
            ]
            return pn.Row(
                density_chart(
