...
def build(runs: list[tuple[str, RunData]], config: Config) -> pn.viewable.Viewable:
    ...
    def _card(metric: str, label: str):
        return kpi_box(
            label=label,
            values=[(run_label, float(tot_df[metric][0]) if metric in tot_df.columns and len(tot_df) > 0 else 0)
    ...
    ptype_chart = bar_chart(
        ...
        x_col="ptype_name", y_col="freq",
        title="Person Type Distribution",
        xaxis_title="Person Type", yaxis_title="Persons",
        pct_col="pct",
    )

    hhsize_chart = bar_chart(
        [(label, df.with_columns(pl.col("HHSIZE").cast(pl.Utf8))) for label, df in hhsize_list],
        x_col="HHSIZE", y_col="freq",
        title="Household Size Distribution",
        xaxis_title="HH Size", yaxis_title="Households",
        pct_col="pct",
    )
    ...
    return pn.Column(
        ...
        pn.widgets.Tabulator(_to_pandas(pct_df), sizing_mode="stretch_width", height=260)
            if len(pct_df) > 0 else pn.pane.Markdown(""),
