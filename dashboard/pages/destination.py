...
import panel as pn
import polars as pl
from dashboard.components import density_chart, data_table, _to_pandas
from summarize.reader import RunData, Config


def _nm_dist_by_purpose(rd: RunData, purpose: str | None, col_name: str) -> pl.DataFrame:
    ...
    # Individual NM and joint NM tours
    ...
    joint = joint.rename({"wgt": "finalweight"}) if "finalweight" not in joint.columns else \
        joint.with_columns(pl.col("wgt").alias("finalweight"))
    ...
    if purpose is None or purpose == "All NM":
        ...
        if "primary_purpose" not in tours.columns:
            return pl.DataFrame({"distbin": list(range(41)), "freq": [0.0] * 41})
        combined = pl.concat([
            indiv.filter(pl.col(col_name) == purpose).select(["SKIMDIST", "finalweight"])
            if "SKIMDIST" in indiv.columns else pl.DataFrame(),
            joint.filter(pl.col(col_name) == purpose).select(["SKIMDIST", "finalweight"])
            if "SKIMDIST" in joint.columns else pl.DataFrame(),
        ])
    ...
    return (combined.group_by("distbin").agg(pl.col("finalweight").sum().alias("freq"))
            .join(pl.DataFrame({"distbin": list(range(41))}), on="distbin", how="right")
            .fill_null(0).sort("distbin"))
...
def build(runs: list[tuple[str, RunData]], config: Config) -> pn.viewable.Viewable:
    ...
    @pn.depends(purp_sel)
    def dist_chart(purp):
        data = [(l, _nm_dist_by_purpose(rd, purp, run_to_purpose_col[l])) for l, rd in runs]
        return density_chart(data, "distbin", "freq",
                             f"NM Tour Distance Distribution — {purp}", "Distance (miles)", normalize=True)
    ...
    return pn.Column(
        ...
        pn.widgets.Tabulator(_to_pandas(avg_df), sizing_mode="stretch_width") if len(avg_df) > 0
