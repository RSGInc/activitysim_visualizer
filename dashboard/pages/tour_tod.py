...
def build(runs: list[tuple[str, RunData]], config: Config) -> pn.viewable.Viewable:
    ...
    def tod_charts(purp):
        ...
        def _prep(df: pl.DataFrame, val_col: str) -> pl.DataFrame:
            return (df.filter(pl.col("purpose") == purp)
        ...
        dur_data = [(l, _prep(df, "freq_dur").with_columns(
            pl.col("timebin").map_elements(lambda tb: _duration_hours(int(tb), maxbin), return_dtype=pl.Float64).alias("duration_hours")
        )) for l, df in tod_list]
        x_label = "Clock time (start at 03:00)"
        dur_plot = density_chart(dur_data, "duration_hours", "freq", f"Duration — {purp}", "Duration (hours)")
        dur_plot.object.update_xaxes(dtick=1, tick0=0, showgrid=True)
        return pn.Column(
            density_chart(dep_data, "clock_time", "freq", f"Departure — {purp}", x_label),
